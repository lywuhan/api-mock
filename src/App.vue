<template>
  <el-container class="app-container">
    ...
    <el-container>
      ...
      <el-main class="main">
        ...
        <div class="main-header">
          ...
          <el-button type="primary" @click="addApi" :disabled="!currentModuleId">
        ...
        <div class="api-container">
          ...
          >
            ...
            <el-table-column label="操作" width="240" fixed="right">
              <template #default="{ row }">
                <el-button type="primary" size="small" @click="editApi(row)">编辑</el-button>
                <el-button type="danger" size="small" @click="deleteApi(row.id)">删除</el-button>
                <el-button type="success" size="small" @click="testApi(row)">测试</el-button>
    ...
    <!-- 模块添加/编辑弹窗 -->
    <el-dialog v-model="moduleDialogVisible" :title="moduleDialogTitle">
    ...
    >
      ...
      <div class="context-menu">
        ...
        <el-button type="text" size="small" @click="deleteModule">删除</el-button>
      </div>
    </el-popover>
                ...
                <template #default="{ node, data }">
                  <div class="tree-node-content">
                    ...
                    <div class="node-actions">
                      <el-button type="text" size="small" @click="editField(data)">
                        <el-icon><Edit /></el-icon> 编辑
                      </el-button>
                      <el-button type="text" size="small" @click="addChildField(data.id)">
                        <el-icon><Plus /></el-icon> 添加子字段
                      </el-button>
                      <el-button type="text" size="small" @click="deleteField(data.id)">
    ...
    <!-- 接口测试弹窗 -->
    <el-dialog v-model="testDialogVisible" :title="'接口测试 - ' + testApiUrl" width="800px">
          ...
          <div class="test-result">
            ...
            <div v-else-if="testResult" class="result-content">
              <div class="result-header">
                <span class="status-code" :class="{'success': testResult.status === 200}">{{ testResult.status }}</span>
      ...
      <el-form :model="fieldForm" label-width="100px">
        ...
        <el-form-item label="Mock规则">
          ...
          <div class="mock-rule-tips">
            <el-button type="text" size="small" @click="insertMockRule('@name')">姓名</el-button>
            <el-button type="text" size="small" @click="insertMockRule('@cname')">中文姓名</el-button>
            <el-button type="text" size="small" @click="insertMockRule('@integer(1, 100)')">数字</el-button>
            <el-button type="text" size="small" @click="insertMockRule('@date')">日期</el-button>
            <el-button type="text" size="small" @click="insertMockRule('@email')">邮箱</el-button>
            <el-button type="text" size="small" @click="insertMockRule('@id')">ID</el-button>
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { apiConfigService, apiTestService, healthCheckService } from './services/apiService'
...
const filteredApis = computed(() => {
  ...
  return apis.value.filter(api => {
    return api.moduleId === currentModuleId.value && 
           (api.name.toLowerCase().includes(query) || api.url.toLowerCase().includes(query))
  })
})
...
const executeTest = async () => {
  ...
  try {
    ...
    const response = await apiTestService.testApi(testForm.url, testForm.method, params)
...
const saveConfigToDB = async () => {
  try {
    await dbService.saveModules(modules.value)
    await dbService.saveApis(apis.value)
    // 同步到后端
...
const loadConfigFromDB = async () => {
  try {
    ...
    if (savedApis.length > 0) {
      ...
      // 同步到后端
      await apiConfigService.updateConfigs(savedApis)
    }
  } catch (error) {
...
// 监听表单树变化，生成Mock数据模板
watch(() => formTreeData.value, (newVal) => {
  if (newVal.length > 0) {
...
const saveField = () => {
  ...
  if (fieldForm.parentId) {
    ...
    if (parentNode) {
      ...
      const existingIndex = parentNode.children.findIndex(node => node.id === fieldForm.id)
      if (existingIndex > -1) {
    ...
    // 添加/编辑根字段
    const existingIndex = formTreeData.value.findIndex(node => node.id === fieldForm.id)
    if (existingIndex > -1) {
